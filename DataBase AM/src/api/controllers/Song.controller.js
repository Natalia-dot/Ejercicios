...
const createSong = async(req, res, next) =>{
    try {
        ...
        }   else return res.status(404).json("The song was not submitted correctly. Please retry.")      
    } catch (error) {
        next(error);
        return (
            res.status(404).json({
  ...
  const getBySongName = async (req, res, next) => {
    try {
      ...
      if (songByName.length > 0) {
        ...
        return res.status(404).json("That song's name doesn't show up in our database.");
      }
    } catch (error) {
      return res.status(404).json({
